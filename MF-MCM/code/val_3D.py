...
def test_all_case_multi(net, base_dir, test_list="full_test.list", num_classes=27, patch_size=(48, 160, 160), stride_xy=32, stride_z=24, select=['t1',None,None,None], val_num=None, if_blank_fill=True, need_sig=False, need_argmax=True):
    with open(os.path.join(base_dir, test_list), 'r') as f:
        image_list = f.readlines()
    if val_num is not None:
        image_list = image_list[:val_num]
    ...
    for image_path in tqdm(image_list):
        ...
        image = np.stack(image,axis=0)
        label = h5f['seg'][:]
        label = one_hot_encoder(input_tensor=label,n_classes=num_classes+1).numpy()
        prediction = test_single_case(
            net, image, stride_xy, stride_z, patch_size, num_classes=num_classes+1, need_sig = need_sig, need_argmax=need_argmax)
        for i in range(num_classes):
            total_metric[i, :] += cal_metric(label[i+1,:,:,:] == 1, prediction[i+1,] == 1)
    print("Validation end")
    return total_metric / len(image_list)
