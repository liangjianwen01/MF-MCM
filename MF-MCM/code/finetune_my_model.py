...
parser.add_argument('--max_iterations', type=int,
                    default=300*210, help='maximum epoch number to train') #need
...
parser.add_argument('--labeled_num', type=int, default=None,
                    help='labeled data') #need
parser.add_argument('--labeled_num_val', type=int, default=None,
                    help='labeled data') #need
...
#                     default='/mnt/liangjw/SSL4MOD/pretrain_balance_model_nnunet_Reconstruct_Coeff_b2:4/BraTs2021_Pre_Train/nnUNet_LY/pretrain_reconstruct+mixcoeff.pth', help='Name of Experiment') #need
parser.add_argument('--pretrain_path', type=str,
                    default=None, help='Name of Experiment') #need
...
def train(args, snapshot_path):
    ...
    model = net_factory_3d(net_type=args.model, in_chns=4, class_num=num_classes).to(device) 
    ...
    db_train = BraTS2021(base_dir=train_data_path,
                         ...
                         select=[None,None,'t2','flair'],
                         transform=transform2,
                         need_brain_mask=False,
                         if_blank_fill=True) #need
    ...
    if args.resume_path is not None:
        ...
        iter_num = int(args.resume_path.split('_')[8].split('.')[0])
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if (iter_num > 0) and ((iter_num+1) % (30*len(trainloader))) == 0: #need
                model.eval()
                avg_metric = test_all_case(
                    model, args.root_path, test_list="val_path_list.txt", num_classes=3, patch_size=args.patch_size,
                    stride_xy=16, stride_z=12, select=[None,None,'t2','flair'], val_num=args.labeled_num_val, if_blank_fill=True, need_sig=True) #need
            ...
            if (iter_num+1) % (50*len(trainloader)) == 0: #need
...
if __name__ == "__main__":
    ...
    snapshot_path = "/mnt/liangjw/SSL4MOD/fullytrain_nnunet_T2&Flair_210/{}/{}".format(args.exp, args.model) #need
