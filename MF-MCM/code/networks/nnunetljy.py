...
class UNet_LY(nn.Module):
    ...
    def __init__(
        ...
        self.reconstruct_out = Convolution(
            self.dimensions,
            32,
            4,
            strides=1,
    ...
    def forward(self, x: torch.Tensor, inference: bool = True) -> torch.Tensor:
        ...
        if self.pretrain:
            if self.reconstruct:
                x_reconstruct = self.reconstruct_out(xu13)
            else:
                x_reconstruct = None

            if self.mix_cof:
                x_mix_cof = self.mix_cof_out(xu13)
            else:
                x_mix_cof = None

            if self.mix_num:
                x_mix_num = self.mix_num_out(xu13)
            else:
                x_mix_num = None

            return x_reconstruct, x_mix_cof, x_mix_num
